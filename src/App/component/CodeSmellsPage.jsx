...
function CodeSmellsPage(prop) {
  ...
  return (
    <div style={{marginLeft: "10px"}}>
      ...
      <h2><a href={codeSmellUrl}
             target="blank">{dataForCodeSmellChart.data.codeSmell[dataForCodeSmellChart.data.codeSmell.length - 1]}</a>
      </h2>
      <div className={classes.root}>
        <div style={{width: "67%"}}>
          <div>
            <h1>Code Smells</h1>
            <div>
              <DrawingBoard data={dataForCodeSmellChart}
                            maxBoardY={Math.max(...dataForCodeSmellChart.data.codeSmell) + 5} id="code-smells-chart"/>
            </div>
          </div>
        </div>
      </div>
